...
class AgentSessionTests(unittest.TestCase):
    ...
    def test_create_runbook_operating_packet_writes_local_packet_only(self) -> None:
        root = self.temp_root()
        workspace_path = root / "workspace"

        result = create_runbook_operating_packet(workspace_path=str(workspace_path))

        self.assertEqual("runbook-operating-packet.v1", result["schema_version"])
    ...
    def test_create_runbook_closeout_packet_reads_existing_runs_only(self) -> None:
        ...
        result = create_runbook_closeout_packet(
            workspace_path=str(workspace_path),
            run_ids_json='["run_design"]',
        )

        payload = json.loads(Path(result["packet_path"]).read_text(encoding="utf-8"))
    ...
    def test_create_release_candidate_audit_reads_release_smoke_only(self) -> None:
        root = self.temp_root()
        workspace_path = root / "workspace"
        run = CompanyGoalRun(
            run_id="run_design",
        ...
        result = create_release_candidate_audit(
            workspace_path=str(workspace_path),
            run_ids_json='["run_design"]',
        )

        payload = json.loads(Path(result["audit_path"]).read_text(encoding="utf-8"))
        self.assertEqual("workroom-release-candidate-audit.v1", result["schema_version"])
        self.assertEqual(["run_design"], payload["run_ids"])
